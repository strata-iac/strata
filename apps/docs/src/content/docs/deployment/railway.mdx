...
<Aside type="note">
Railway auto-detects the `Dockerfile` even without `builder = "DOCKERFILE"`, but specifying it explicitly is cleaner. The Dockerfile's `ENTRYPOINT ["/procella"]` is used by default — only add `startCommand` if you need to override it.
</Aside>
